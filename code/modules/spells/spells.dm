	...
	if(. && ability_master && ability_master.spell_objects)
		for(var/obj/screen/ability/spell/screen in ability_master.spell_objects)
			...
			switch(S.charge_type)
				if(Sp_RECHARGE)
					. += "[S.charge_counter/10.0]/[S.charge_max/10]" + S.connected_button
				if(Sp_CHARGES)
					. += "[S.charge_counter]/[S.charge_max]" + S.connected_button
				if(Sp_HOLDVAR)
					. += "[S.holder_var_type] [S.holder_var_amount]" + S.connected_button
