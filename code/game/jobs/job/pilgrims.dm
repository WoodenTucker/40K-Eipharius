...
/mob/living/carbon/human/proc/penitentclass()
	...
	switch(classchoice)
		...
		if("Bounty Hunter")
			...
			U.add_skills(rand(5,8),rand(5,8),rand(1,3),rand(1,2),rand(1,2))
			equip_to_slot_or_del(new /obj/item/storage/backpack/satchel/warfare, slot_back)
		...
		if("Miner")
			...
			U.verbs -= list(/mob/living/carbon/human/proc/penitentclass,)
		if("Stalker")
			U.add_skills(rand(1,3),rand(5,8),rand(1,2),rand(1,2),rand(1,2))
		...
		if("Disgraced Medicae")
			U.add_skills(rand(1,3),rand(2,3),rand(5,9),rand(1,2),rand(5,9))
