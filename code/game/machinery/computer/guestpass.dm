...
/obj/item/card/id/guest/examine(mob/user)
	...
	if (world.time < expiration_time)
		...
		to_chat(user, SPAN_NOTICE("This pass expired at [time2text(SSticker.round_start_time + expiration_time, "hh:mm")]."))

/obj/item/card/id/guest/read(mob/user)
	if (world.time > expiration_time)
		to_chat(user, SPAN_NOTICE("This pass expired at [time2text(SSticker.round_start_time + expiration_time, "hh:mm")]."))
	else
		to_chat(user, SPAN_NOTICE("This pass expired at [time2text(SSticker.round_start_time + expiration_time, "hh:mm")]."))
...
//				to_chat(user, "Printing the log, standby...")
			...
			if ("issue")
				if (giver && accesses.len)
					...
					entry += ". Expires at [time2text(SSticker.round_start_time + world.time + duration MINUTES, "hh:mm")]."
