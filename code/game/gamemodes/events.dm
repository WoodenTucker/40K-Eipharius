...
*/
	...
	for(var/mob/living/silicon/ai/M in GLOB.living_mob_list_)
		if(M.stat != 2 && M.see_in_dark != 0)
			...
			var/target = pick("a traitor", "a syndicate agent", "a changeling", "a wizard", "the head of a revolution", "Soviet spy", "a good person", "a dwarf", "an elf", "a fairy princess", "the Rogue Trader", "Beepsky", "God", "a pirate", "a gryphon", "a chryssalid")
			...
			switch(rand(1,14))
				...
				if(5)
					to_chat(M, "<br>")
					to_chat(M, "<span class='danger'>the Rogue Trader, HOP AND HOS ARE [who2]...LAWS UPDATED</span>")
					to_chat(M, "<br>")
					M.add_ion_law("the Rogue Trader, HOP AND HOS ARE [who2]")
				if(6)
					to_chat(M, "<br>")
