...
/datum/game_mode/meteor/post_setup()
	...
	next_wave = (world.time - SSticker.round_start_time) + METEOR_DELAY
...
/datum/game_mode/meteor/process()
	...
	if(((world.time - SSticker.round_start_time) >= next_wave) && (alert_sent == 1))
		...
		next_wave = (world.time - SSticker.round_start_time) + (meteor_wave_delay * time_between_waves_minutes)
